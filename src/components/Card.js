...
function Card(props) {
    return (
        <div className="d-inline-block my-card" onClick={e =>props.click(props.item)}>
            ...
            {props.item.selected && <CardWords title={props.item.title}  subTitle={props.item.subTitle} tools={props.item.tools} gitLink={props.item.gitLink} link={props.item.link}/>}
        </div>
    )
}
